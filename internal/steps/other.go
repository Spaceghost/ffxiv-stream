...
func (b *builder) wolf() ([]plan.Step, error) {
	...
	s = append(s,
		...
		plan.File(host, "/etc/modules-load.d/ffxiv-stream-uhid.conf", []byte("uhid\n"), 0o644, "", "Load uhid at boot", "Otherwise it loads on demand and virtual pads fail with permission errors."),
		plan.File(host, "/etc/wolf/cfg/config.toml", wolfConfig(c), 0o644, "", "Write Wolf's app list", "One app: XIVLauncher in ffxiv-stream's app image."),
...
const WolfImage = "ghcr.io/spaceghost/ffxiv-stream-xivlauncher:latest"

func wolfConfig(c config.Config) []byte {
	return []byte(fmt.Sprintf(`# ffxiv-stream: Wolf's apps. Wolf reads this at start and rewrites it only
# when a client pairs; stop Wolf before editing by hand. (Generated by ffxiv-stream.)
[[profiles]]
...
// /run/udev for the virtual devices, and the API socket for `ffxiv-stream pair`.
func wolfQuadlet(c config.Config, renderNode string, nvidia bool) []byte {
	...
	return []byte(`# ffxiv-stream: Wolf, rootful. (Generated by ffxiv-stream.)
[Unit]
Description=Wolf game streaming server (ffxiv-stream)
